...
“编译本机 EXE”依次执行前端构建和带 `embed` 标记的 Go 构建，生成 `C:\ProgramData\Sub2API\builds\sub2api-时间戳.exe`。编译窗口会保留供查看结果。它不会覆盖 `backend/sub2api.exe`，也不会重启 18271 或 18272；新页面需在你手动替换程序并重启目标实例后才会生效。
